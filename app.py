...
# ── ML integration ─────────────────────────────────────────────────────────────
try:
    from ml_threat_scorer import ThreatScorer as _ThreatScorer
    _ml_threat_scorer = _ThreatScorer()
except Exception:
    _ml_threat_scorer = None
# ───────────────────────────────────────────────────────────────────────────────
...
def log_callback(message: str) -> None:
    ...
    # Persist to MySQL
    honeypot_db.insert_log(message)
...
@login_required
def get_db_logs():
    """Return honeypot logs stored in SQLite.

    Query parameters:
      limit   - max rows to return (default 500)
      service - filter by service: http / ftp / ssh
    """
    limit = min(int(request.args.get("limit", 500)), 5000)
    service = request.args.get("service") or None
    logs = honeypot_db.get_logs(limit=limit, service=service)
    return jsonify({"logs": logs, "count": len(logs), "db_available": honeypot_db.is_available()})


@app.route("/api/ml/threat-scores", methods=["GET"])
@login_required
def get_ml_threat_scores():
    """
    ML Threat Scores endpoint.
    Returns per-IP threat scores (0-100) computed by the Isolation Forest model.

    Query parameters:
      limit - max log rows to analyse (default 1000)
    """
    if _ml_threat_scorer is None:
        return jsonify({"error": "ML threat scorer unavailable", "scores": {}}), 503

    limit = min(int(request.args.get("limit", 1000)), 5000)
    logs = honeypot_db.get_logs(limit=limit)
    scores = _ml_threat_scorer.score_all_ips(logs)

    result = [
        {
            "ip": ip,
            "score": score,
            "threat_level": _ml_threat_scorer.threat_level(score),
        }
        for ip, score in sorted(scores.items(), key=lambda x: x[1], reverse=True)
    ]
    return jsonify({"scores": result, "total_ips": len(result), "ml_ready": _ml_threat_scorer.is_ready})
