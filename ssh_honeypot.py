...
try:
    from ml_command_intent import CommandIntentClassifier as _CommandIntentClassifier
    _ml_cmd_clf = _CommandIntentClassifier()
except Exception:
    _ml_cmd_clf = None
# ───────────────────────────────────────────────────────────────────────────────
...
class _HoneypotServer(paramiko.ServerInterface):
    ...
    def check_auth_password(self, username, password):
        ...
        # ── ML Brute Force Detection ───────────────────────────────────────────
        if _ml_bf_detector is not None:
            result = _ml_bf_detector.record_attempt(self.client_ip, username, password)
            attempts = result.get("attempt_count", 0)
            confidence = result.get("confidence", 0.0)
            is_bf = result.get("is_brute_force", False)
            self.hp.log(
                f"[ML Brute Force] IP={self.client_ip} attempt={attempts} "
...
class SSHHoneypot:
    ...
    def _run_command(self, command, cwd, client_ip):
        ...
        # ── ML Command Intent Classification ──────────────────────────────────
        if _ml_cmd_clf is not None and _ml_cmd_clf.is_ready:
            intent, conf = _ml_cmd_clf.predict(command)
            label = intent if intent else "Normal"
            self.log(f"[ML CmdIntent] IP={client_ip} intent={label} confidence={conf:.0%} command={command}")
        # ─────────────────────────────────────────────────────────────────────
